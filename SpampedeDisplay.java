...
class SpampedeDisplay {
	...
	public void updateGraphics() {
		...
		int xShift = (this.width-Preferences.CELL_SIZE*this.theData.getNumColumns())/2;
		int yShift = Preferences.TITLE_Y+15;
		for (int p = 0; p < Preferences.CELL_SIZE*this.theData.getNumRows(); p+=Preferences.CELL_SIZE) {
			for (int q = 0; q < Preferences.CELL_SIZE*this.theData.getNumColumns(); q+=Preferences.CELL_SIZE) {
				Color cellColor = this.theData.getCellColor(p/Preferences.CELL_SIZE, q/Preferences.CELL_SIZE);
				drawSquare(q + xShift, p + yShift,cellColor);
			}
		}
